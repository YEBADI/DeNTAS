...
files <- c('10s0r1.uniq.txt', '11s0r2.uniq.txt', '12s0r3.uniq.txt',
           '13s8r1.uniq.txt','14s8r2.uniq.txt', '15s8r3.uniq.txt',
            '16s24r1.uniq.txt', '17s24r2.uniq.txt', '18s24r3.uniq.txt')
...
genes <- read.table(file="functions3.txt", fill = TRUE)
...
df <- cbind(df, GeneID)
write.table(df, file = "results/tables/data_matrix.txt")
...
### PCA plot
pdf('results/graphs/PCA_all_data.pdf')
lfpkm <- t(x$counts)
Xpca <-prcomp(lfpkm)
Xscores   <- Xpca$x
...
### HCA dendrogram
pdf('results/graphs/HC_all_data.pdf')
...
write.table(de_UvsT8, file="results/tables/de_UvsT8.txt", row.names=FALSE)
write.table(de_UvsT24, file="results/tables/de_UvsT24.txt", row.names=FALSE)
write.table(de_T8vsT24, file="results/tables/de_T8vsT24.txt", row.names=FALSE)
write.table(de_all, file="results/tables/de_all.txt", row.names=FALSE)

### this writes out a table describing the limma derived model
write.fit(efit, dt, file="results/tables/de_limma_model.txt")
...
pdf('results/graphs/Uninf_vs_H_8hrs_Volcano.pdf')
...
pdf('results/graphs/Uninf_vs_H_24hrs_Volcano.pdf')
...
pdf('results/graphs/H_8hrs_vs_H_24hrs_Volcano.pdf')
...
pdf('results/graphs/Topgenes_Heatmap.pdf')
...
### PCA plot
pdf('results/graphs/PCA_topgenes.pdf')
...
### HCA dendrogram
pdf('results/graphs/HC_topgenes.pdf')
hc <-hclust(dist(lfpkm))
dend <-as.dendrogram(hc)
